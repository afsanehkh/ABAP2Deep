...
CLASS lcl_passenger_flight IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    SELECT
      ...
      FIELDS carrier_id, connection_id, airport_from_id, airport_to_id, departure_time, arrival_time, f~timzone AS timzone_from, t~timzone AS timzone_to,
      div(
  ...
  METHOD get_flights_by_carrier.
    ...
    IF NOT line_exists( flights_buffer[
      ...
      SELECT
        FROM /lrn/passflight
      FIELDS carrier_id, connection_id, flight_date, plane_type_id, seats_max, seats_occupied, seats_max - seats_occupied AS seats_free, @currency AS currency_code,
          currency_conversion( amount             = price,
               source_currency    = currency_code,
               target_currency    = @currency,
               exchange_rate_date = flight_date,
               on_error           =
                 @sql_currency_conversion=>c_on_error-set_to_null ) AS price
    ...
    r_result = VALUE #( FOR <flight> IN flights_buffer
                        ...
                        WHERE ( carrier_id = i_carrier_id ) ( NEW lcl_passenger_flight(
...
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! @raising zcx_2147_failed | Instantiation failed - evaluate the exception text for details.
    ...
    "! @parameter i_seats | Minimum number of seats
    "! @parameter e_flight | Reference to the flight instance
    "! @parameter e_days_later | Number of days the flight is later than the requested date
...
*    INTO ( @me->name, @me->currency_code ).
    ...
    IF sy-subrc <> 0.
       ...
       RAISE EXCEPTION TYPE zcx_2147_failed
         EXPORTING
           textid     = zcx_2147_failed=>carrier_not_exist
*           previous   =
...
*
        RAISE EXCEPTION TYPE zcx_2147_failed
          EXPORTING
            textid     = zcx_2147_failed=>carrier_no_read_auth
*           previous   =
            carrier_id = i_carrier_id.
